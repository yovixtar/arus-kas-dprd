...
$cek_data = mysqli_num_rows(mysqli_query($l, "SELECT * FROM nota n JOIN kegiatan k ON n.id_kegiatan=k.id_kegiatan WHERE tanggal_nota BETWEEN '".$date_start."' AND '".$date_end."' ORDER BY id_nota ASC LIMIT 1"));
if ($cek_data > 0) {
    
$data_limit_select = mysqli_fetch_array(mysqli_query($l, "SELECT * FROM nota n JOIN kegiatan k ON n.id_kegiatan=k.id_kegiatan WHERE tanggal_nota BETWEEN '".$date_start."' AND '".$date_end."' ORDER BY id_nota ASC LIMIT 1"));
$limit_select = $data_limit_select['id_nota'] - 1;
...
$stat_select_nota_1 = mysqli_query($l, "SELECT * FROM nota n JOIN kegiatan k ON n.id_kegiatan=k.id_kegiatan WHERE tanggal_nota BETWEEN '".$date_start."' AND '".$date_end."' ORDER BY id_nota ASC");
?>
<table id="table">
  <tr>
    <th class="no">No</th>
  ...
  <?php
  if ($data_limit_select['id_nota'] != 1) {
    $show_saldo = 0;
...
<?php
$data_jumlah_awal = mysqli_fetch_array(mysqli_query($l, "SELECT * FROM nota WHERE tanggal_nota BETWEEN '".$date_start."' AND '".$date_end."' ORDER BY id_nota ASC LIMIT 1"));
$data_jumlah_akhir = mysqli_fetch_array(mysqli_query($l, "SELECT * FROM nota WHERE tanggal_nota BETWEEN '".$date_start."' AND '".$date_end."' ORDER BY id_nota DESC LIMIT 1"));

$stat_jumlah = mysqli_query($l, "SELECT SUM(count_penerimaan) AS Tpenerimaan, NULL AS Tpengeluaran FROM penerimaan WHERE id_nota BETWEEN ".$data_jumlah_awal['id_nota']." AND ".$data_jumlah_akhir['id_nota']." UNION SELECT NULL AS Tpenerimaan, SUM(count_pengeluaran) AS Tpengeluaran FROM pengeluaran WHERE id_nota BETWEEN ".$data_jumlah_awal['id_nota']." AND ".$data_jumlah_akhir['id_nota']);
